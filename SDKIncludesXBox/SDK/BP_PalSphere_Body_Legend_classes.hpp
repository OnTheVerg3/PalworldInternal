#pragma once

/*
* SDK generated by Dumper-7
*
* https://github.com/Encryqed/Dumper-7
*/

// Package: BP_PalSphere_Body_Legend

#include "Basic.hpp"

#include "Engine_structs.hpp"
#include "BP_PalSphere_Body_classes.hpp"


namespace SDK
{

// BlueprintGeneratedClass BP_PalSphere_Body_Legend.BP_PalSphere_Body_Legend_C
// 0x0008 (0x0490 - 0x0488)
class ABP_PalSphere_Body_Legend_C final : public ABP_PalSphere_Body_C
{
public:
	struct FPointerToUberGraphFrame               UberGraphFrame_BP_PalSphere_Body_Legend_C;         // 0x0488(0x0008)(ZeroConstructor, Transient, DuplicateTransient)

public:
	void ExecuteUbergraph_BP_PalSphere_Body_Legend(int32 EntryPoint);
	void ReceiveBeginPlay();

public:
	static class UClass* StaticClass()
	{
		return StaticBPGeneratedClassImpl<"BP_PalSphere_Body_Legend_C">();
	}
	static class ABP_PalSphere_Body_Legend_C* GetDefaultObj()
	{
		return GetDefaultObjImpl<ABP_PalSphere_Body_Legend_C>();
	}
};
static_assert(alignof(ABP_PalSphere_Body_Legend_C) == 0x000008, "Wrong alignment on ABP_PalSphere_Body_Legend_C");
static_assert(sizeof(ABP_PalSphere_Body_Legend_C) == 0x000490, "Wrong size on ABP_PalSphere_Body_Legend_C");
static_assert(offsetof(ABP_PalSphere_Body_Legend_C, UberGraphFrame_BP_PalSphere_Body_Legend_C) == 0x000488, "Member 'ABP_PalSphere_Body_Legend_C::UberGraphFrame_BP_PalSphere_Body_Legend_C' has a wrong offset!");

}

