#pragma once

/*
* SDK generated by Dumper-7
*
* https://github.com/Encryqed/Dumper-7
*/

// Package: BP_Glider_Item_Base

#include "Basic.hpp"

#include "BP_GliderObjectBase_classes.hpp"


namespace SDK
{

// BlueprintGeneratedClass BP_Glider_Item_Base.BP_Glider_Item_Base_C
// 0x0008 (0x0350 - 0x0348)
class ABP_Glider_Item_Base_C final : public ABP_GliderObjectBase_C
{
public:
	class USkeletalMeshComponent*                 SkeletalMesh;                                      // 0x0348(0x0008)(BlueprintVisible, ZeroConstructor, InstancedReference, NonTransactional, NoDestructor, HasGetValueTypeHash)

public:
	static class UClass* StaticClass()
	{
		return StaticBPGeneratedClassImpl<"BP_Glider_Item_Base_C">();
	}
	static class ABP_Glider_Item_Base_C* GetDefaultObj()
	{
		return GetDefaultObjImpl<ABP_Glider_Item_Base_C>();
	}
};
static_assert(alignof(ABP_Glider_Item_Base_C) == 0x000008, "Wrong alignment on ABP_Glider_Item_Base_C");
static_assert(sizeof(ABP_Glider_Item_Base_C) == 0x000350, "Wrong size on ABP_Glider_Item_Base_C");
static_assert(offsetof(ABP_Glider_Item_Base_C, SkeletalMesh) == 0x000348, "Member 'ABP_Glider_Item_Base_C::SkeletalMesh' has a wrong offset!");

}

